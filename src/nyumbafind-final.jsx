...
function PaymentModal({ house, onClose }) {
  ...
  const pay = async () => {
    ...
    // HTML Form POST to PayChangu — official method from their docs
    ...
    const fields = {
      ...
      meta:         JSON.stringify({ tx_ref: ref, listing_id: house.id }),
    };
    ...
    for (const file of photos) {
      try {
        ...
        const res  = await fetch(
          `${process.env.REACT_APP_SUPABASE_URL}/storage/v1/object/house-photos/${name}`,
          { method:"POST", headers:{ apikey:process.env.REACT_APP_SUPABASE_KEY, Authorization:`Bearer ${process.env.REACT_APP_SUPABASE_KEY}`, "Content-Type":file.type, "x-upsert":"true" }, body:file }
        );
        if (res.ok) urls.push(`${process.env.REACT_APP_SUPABASE_URL}/storage/v1/object/public/house-photos/${name}`);
  ...
  const submit = async () => {
    ...
    try {
      ...
      await supabase.insert("listings",{
        ...
        landlord_name: sanitize(f.landlord_name),
        landlord_phone:sanitize(f.landlord_phone),
        photos:        photoUrls,
        status:        "pending",
        id_checked:    false,
        chief_verified:false,
      });
      setDone(true);
  ...
  if (done) return (
    <div style={{ maxWidth:500,margin:"60px auto",padding:24,textAlign:"center" }}>
      ...
      <div style={{ background:"#f0fdf4",borderRadius:13,padding:15,marginTop:18,textAlign:"left",fontSize:".81rem",color:"#15803d",lineHeight:1.9 }}>
        <strong>Prepare:</strong><br/>🪪 National ID &nbsp;•&nbsp; 👑 Chief's letter &nbsp;•&nbsp; 📸 Photos uploaded ✅ &nbsp;•&nbsp; 📄 Proof of ownership
      </div>
    </div>
  );
    ...
    <div style={{ maxWidth:560,margin:"0 auto",padding:"34px 20px" }}>
      ...
      <div style={{ background:"#fff",borderRadius:20,padding:24,boxShadow:"0 4px 24px rgba(0,0,0,.08)" }}>
        ...
        <div style={{ marginBottom:18 }}>
          <label style={{ display:"block",fontWeight:600,marginBottom:6,color:"#374151",fontSize:".83rem" }}>
            📸 Property Photos <span style={{ color:"#9ca3af",fontWeight:400,fontSize:".76rem" }}>— up to 5 photos</span>
          </label>

          {/* Upload area */}
          <label style={{ display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:6,background:"#f0fdf4",border:"2px dashed #86efac",borderRadius:14,padding:"22px 16px",cursor:"pointer",transition:"all .2s",textAlign:"center" }}
            ...
            <div style={{ fontWeight:700,color:"#15803d",fontSize:".9rem" }}>Tap here to add photos</div>
            <div style={{ color:"#9ca3af",fontSize:".75rem" }}>JPG or PNG • Up to 5 photos • Tenants will see these</div>
          </label>
            ...
            <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(90px,1fr))",gap:8,marginTop:12 }}>
              {previews.map((url,i)=>(
                <div key={i} style={{ position:"relative",borderRadius:10,overflow:"hidden",aspectRatio:"1",border:`2px solid ${i===0?"#15803d":"#bbf7d0"}` }}>
                  ...
                  {i===0&&<div style={{ position:"absolute",bottom:0,left:0,right:0,background:"rgba(21,128,61,.85)",color:"#fff",fontSize:".58rem",textAlign:"center",padding:"2px 0",fontWeight:700,letterSpacing:".04em" }}>MAIN PHOTO</div>}
                </div>
              ))}
              {previews.length<5&&(
                <label style={{ border:"2px dashed #d1fae5",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",aspectRatio:"1",cursor:"pointer",background:"#f9fafb",flexDirection:"column",gap:4 }}>
                  <input type="file" accept="image/*" multiple onChange={handlePhotos} style={{ display:"none" }}/>
                  <span style={{ color:"#9ca3af",fontSize:"1.6rem",lineHeight:1 }}>+</span>
                  <span style={{ color:"#9ca3af",fontSize:".6rem" }}>Add more</span>
                </label>
              )}
            </div>
          )}

          {uploading&&<div style={{ textAlign:"center",padding:"10px 0",color:"#15803d",fontSize:".82rem",fontWeight:600 }}>⏳ Uploading photos... please wait</div>}
          {photos.length>0&&!uploading&&<div style={{ color:"#15803d",fontSize:".76rem",marginTop:6,fontWeight:600 }}>✅ {photos.length} photo{photos.length>1?"s":""} selected — will upload when you submit</div>}
        </div>
