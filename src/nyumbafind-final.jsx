            ...
            <div>
              ...
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:2 }}>
                <div>
                  ...
                  <input
                    ...
                    value={fname}
                    onChange={e=>{setFname(e.target.value);touch("fname",e.target.value);}}
                ...
                <div>
                  ...
                  <input
                    ...
                    value={lname}
                    onChange={e=>{setLname(e.target.value);touch("lname",e.target.value);}}
              ...
              <div style={{ marginBottom:2,marginTop:11 }}>
                ...
                <input
                  ...
                  value={email}
                  onChange={e=>{setEmail(e.target.value);touch("email",e.target.value);}}
...
function InstallBanner() {
  ...
  return (
    <div style={{ position:"fixed", bottom:0, left:0, right:0, zIndex:9999, animation:"fadeUp .4s ease" }}>
      <div style={{ background:"linear-gradient(135deg,#052e16,#15803d)", padding:"16px 20px", display:"flex", alignItems:"center", gap:14, boxShadow:"0 -4px 24px rgba(0,0,0,.25)" }}>
...
export default function NyumbaFind() {
  ...
  return (
    <div style={{ minHeight:"100vh",background:"#f8fdf9",fontFamily:"'DM Sans',sans-serif" }}>
      ...
      {/* NAV */}
      <nav style={{ background:"rgba(255,255,255,.93)",backdropFilter:"blur(12px)",borderBottom:"1px solid #dcfce7",padding:"0 20px",height:62,display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:500 }}>
        <div style={{ display:"flex",alignItems:"center",gap:7,cursor:"pointer" }} onClick={()=>setView("home")}>
          ...
          <span style={{ fontFamily:"'Playfair Display',serif",fontWeight:900,fontSize:"1.2rem" }}>Nyumba<span style={{ color:"#15803d" }}>Find</span></span>
        </div>
        <div style={{ display:"flex",gap:2,flexWrap:"wrap" }}>
          <Nav label="Home"          v="home"/>
          <Nav label="Browse"        v="listings"/>
          <Nav label="How It Works"  v="how"/>
          <Nav label={`Saved${saved.length?` (${saved.length})`:""}`} v="saved"/>
        ...
        <div>
          <div style={{ background:"linear-gradient(135deg,#052e16,#14532d,#15803d)",padding:"70px 22px 62px",textAlign:"center",position:"relative",overflow:"hidden" }}>
            ...
            <div style={{ position:"relative",maxWidth:680,margin:"0 auto",animation:"fadeUp .7s ease" }}>
              ...
              <div style={{ background:"#fff",borderRadius:15,padding:7,display:"flex",gap:7,maxWidth:520,margin:"0 auto",boxShadow:"0 20px 60px rgba(0,0,0,.3)" }}>
                <input placeholder="🔍 Search city or neighbourhood..." value={search} onChange={e=>setSearch(e.target.value)} style={{ flex:1,border:"none",padding:"10px 13px",fontSize:".9rem",borderRadius:9,background:"transparent",color:"#111",outline:"none" }}/>
                <button onClick={()=>setView("listings")} style={{ background:"linear-gradient(135deg,#15803d,#4ade80)",color:"#fff",border:"none",borderRadius:9,padding:"10px 18px",fontWeight:700,cursor:"pointer" }}>Search</button>
              </div>
          ...
          {loading&&<Spinner/>}
          {!loading&&err&&<div style={{ maxWidth:580,margin:"30px auto",padding:"0 20px" }}><ErrMsg msg={err}/><p style={{ color:"#6b7280",textAlign:"center",fontSize:".84rem" }}>Make sure your Supabase tables are created and RLS policies allow public reads.</p></div>}
      ...
      {view==="listings"&&(
        <div style={{ maxWidth:1060,margin:"0 auto",padding:"24px 20px" }}>
          ...
          <div style={{ background:"#fff",borderRadius:13,padding:15,marginBottom:18,display:"flex",gap:9,flexWrap:"wrap",boxShadow:"0 2px 10px rgba(0,0,0,.06)" }}>
      ...
      <footer style={{ background:"#052e16",color:"#86efac",textAlign:"center",padding:"18px",fontSize:".77rem",marginTop:40 }}>
        ...
        <p style={{ margin:0,opacity:.5 }}>Connected to Supabase · PayChangu · 🇲🇼 Malawi</p>
      </footer>
      <Toast msg={toast}/>
      <InstallBanner/>
    </div>
