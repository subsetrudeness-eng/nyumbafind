...
// ─── SECURITY ────────────────────────────────────────────────────────────────
// Sanitize all inputs — strips SQL injection & XSS attacks
const sanitize = (str) => {
  if (!str) return "";
  return String(str)
    .replace(/['";\-\-\/\*]/g, "")
    .replace(/<[^>]*>/g, "")
    .replace(/\b(SELECT|INSERT|UPDATE|DELETE|DROP|EXEC|UNION)\b/gi, "")
    .trim()
    .slice(0, 500);
};

// Brute force protection — max 5 wrong passwords in 15 minutes
const loginLimiter = (() => {
  let attempts = [];
  ...
  return {
    canTry: () => {
      ...
      if (attempts.length >= 5) {
        ...
        return "Too many failed attempts. Locked for 15 minutes.";
      }
      return null;
    },
    recordFail: () => { attempts.push(Date.now()); },
    reset: () => { attempts = []; lockedUntil = null; },
  };
})();

// ─── REAL SUPABASE CONNECTION ─────────────────────────────────────────────────
// ── Direct values — guaranteed to work ──────────────────────────────────────
const SUPABASE_URL   = process.env.REACT_APP_SUPABASE_URL   || "https://uhkfesqvkxrajaaztuch.supabase.co";
const SUPABASE_KEY   = process.env.REACT_APP_SUPABASE_KEY   || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InVoa2Zlc3F2a3hyYWphYXp0dWNoIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODA2ODY5NzYsImV4cCI6MjA5NjI2Mjk3Nn0.DZMd6JrFaPY_22rvSWpQ0JkVVC-3tl4emIyW6kOhSqc";
const DYLAN_WA       = process.env.REACT_APP_DYLAN_WA       || "265987596070";
...
const db = {
  ...
  patch: async (table, data, filter) => {
    ...
    const text = await r.text();
    if (!r.ok) throw new Error(`PATCH failed (${r.status}): ${text}`);
  ...
  post: async (table, data) => {
    ...
    const text = await r.text();
    if (!r.ok) throw new Error(`POST failed (${r.status}): ${text}`);
    return text ? JSON.parse(text) : {};
  },
  del: async (table, filter) => {
    ...
    if (!r.ok) { const text = await r.text(); throw new Error(`DELETE failed (${r.status}): ${text}`); }
    return true;
  },
};
...
function Listings({ listings, setListings, showToast, showConfirm, loading, err, reload }) {
  ...
  const approve = (l) => showConfirm({
    ...
    if (!ok) return;
    try {
      console.log("Approving listing:", l.id);
      const result = await db.patch("listings", { status:"active", id_checked:true, chief_verified:true }, `id=eq.${l.id}`);
      ...
      showToast(null, `Approve failed: ${e.message}`);
    }
  });
...
export default function AdminApp() {
  ...
  const login = () => {
    ...
    const blocked = loginLimiter.canTry();
    if (blocked) { setLoginErr(blocked); return; }

    // Sanitize inputs
    const cleanEmail = sanitize(email).toLowerCase();
    const cleanPass  = sanitize(pass);

    if (cleanEmail==="admin@nyumbafind.mw" && cleanPass===ADMIN_PASSWORD) {
      loginLimiter.reset();
      setLoggedIn(true);
      setLoginErr("");
    } else {
      loginLimiter.recordFail();
    ...
    <div style={{ minHeight:"100vh",background:"linear-gradient(135deg,#052e16,#14532d,#15803d)",display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div style={{ background:"#fff",borderRadius:24,maxWidth:400,width:"100%",overflow:"hidden",boxShadow:"0 24px 80px rgba(0,0,0,.35)",animation:"fadeUp .5s ease" }}>
        ...
        <div style={{ padding:"24px" }}>
          ...
          <div style={{ background:"#f0fdf4",borderRadius:10,padding:10,marginTop:13,fontSize:".72rem",color:"#15803d",textAlign:"center",lineHeight:1.7 }}>🔑 admin@nyumbafind.mw &nbsp;/&nbsp; your password</div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="admin-layout" style={{ minHeight:"100vh",background:"#f8fdf9",fontFamily:"'DM Sans',sans-serif",display:"flex" }}>
      <style>{`
        ...
        *{box-sizing:border-box} ::-webkit-scrollbar{width:5px} ::-webkit-scrollbar-thumb{background:#d1fae5;border-radius:3px}
        ...
        @media (max-width: 768px) {
          ...
          .admin-sidebar { width: 100% !important; height: auto !important; min-height: unset !important; position: relative !important; flex-direction: row !important; flex-wrap: wrap !important; padding: 10px !important; gap: 6px !important; }
          .admin-sidebar .sidebar-logo  { display: none !important; }
          .admin-sidebar .sidebar-footer { display: none !important; }
          .admin-content { max-height: unset !important; padding: 16px !important; }
          .admin-nav-btn { padding: 7px 10px !important; font-size: .72rem !important; flex:1 !important; justify-content: center !important; }
        ...
        @media (max-width: 400px) {
          ...
          .admin-nav-btn span:last-child { display: none !important; }
        }
      `}</style>
